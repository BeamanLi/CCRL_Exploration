...
class Runner:
    ...
    def switch_map(self):
        ...
        vector_maps = [lambda: gym.make("ExploreEnv-{}".format(self.env_version), map_name=self.map_names[self.map_index_now], random_obstacle=self.random_obstacle, training=True)] * self.num_envs_per_map * (self.map_index_now + 1)
        self.envs = gym.vector.AsyncVectorEnv(vector_maps)
        self.env_evaluate.append(gym.make('ExploreEnv-{}'.format(self.env_version), map_name=self.map_names[self.map_index_now], random_obstacle=self.random_obstacle, training=False))  # 新增加一个评估地图
        ...
        self.evaluate_single_map(self.map_index_now, self.env_evaluate[-1])
        return s
    ...
    def evaluate_policy(self):
        ...
        for map_index, env in enumerate(self.env_evaluate):  # The policy is evaluated simultaneously on all previously experienced levels
            self.evaluate_single_map(map_index, env)
        if self.save_model:
    ...
    def evaluate_single_map(self, map_index, env):
        evaluate_reward = 0
        ...
        self.writer.add_scalar('vectorized_step_rewards_{}'.format(self.map_names[map_index]), evaluate_reward, global_step=self.step_times)
        self.writer.add_scalar('vectorized_step_explore_rate_{}'.format(self.map_names[map_index]), evaluate_explore_rate, global_step=self.step_times)
        print("step_times:{} \t map_name:{} \t reward:{} \t explore_rate:{}".format(self.step_times, self.map_names[map_index], evaluate_reward, evaluate_explore_rate))
