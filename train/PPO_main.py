...
class Runner:
    def __init__(self, config):
        ...
        self.env_evaluate = [gym.make('ExploreEnv-{}'.format(self.env_version), map_name=map_name, random_obstacle=self.random_obstacle, training=False) for map_name in self.map_names]
        ...
        self.total_steps_per_map = np.zeros(len(self.map_names), dtype=np.int32)  # Total transitions sample on per map
        self.delta_steps = np.ones(len(self.map_names), dtype=np.int32) * self.num_envs_per_map
    ...
    def evaluate_policy(self):
        self.evaluate_num += 1
        for map_index, env in enumerate(self.env_evaluate):  # The policy is evaluated simultaneously on all levels
